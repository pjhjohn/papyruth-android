...
public class Error503 {
    private static boolean report2GoogleAnalytics(RetrofitError throwable, Object object, boolean toast) {
        if (toast && object instanceof Context) Toast.makeText((Context) object, R.string.toast_error_retrofit_503, Toast.LENGTH_SHORT).show();
    ...
    public static ErrorHandleResult handle(RetrofitError throwable, Object object, boolean toast) {
        if(throwable.getResponse().getStatus() != 503) return new ErrorHandleResult(false);
