...
package com.papyruth.support.opensource.materialprogressbar;
...
import com.papyruth.support.opensource.materialprogressbar.internal.ObjectAnimatorCompat;
...
class Animators {
    ...
    public static Animator createIndeterminateColorScheme(final IndeterminateProgressDrawable target, int[] colors) {
        List<Animator> animators = new ArrayList<>();
        for(int i = 0; i < colors.length; i ++) {
            ValueAnimator animator = new ValueAnimator();
            animator.setIntValues(colors[i], colors[(i + 1) % colors.length]);
            animator.setEvaluator(new ArgbEvaluator());
            animator.setInterpolator(new DecelerateInterpolator(6));
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    target.setColorFilter((int) animation.getAnimatedValue(), PorterDuff.Mode.SRC_ATOP);
                }
            });
